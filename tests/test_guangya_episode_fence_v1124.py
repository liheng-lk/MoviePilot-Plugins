...
class GuangYaEpisodeFenceV1124Tests(unittest.TestCase):
    ...
    def test_sources_parse_and_fence_is_in_runtime_mro(self):
        ...
        self.assertNotIn("episode_fence_final_v1124", self.entry)
        self.assertNotIn("GuangYaEpisodeFenceFinalV1124Mixin", self.entry)
        self.assertLess(
            self.entry.index("    GuangYaReleaseV1110Mixin,"),
            self.entry.index("    GuangYaReceiptCompletionV1124Mixin,"),
        )
        self.assertIn("build_id = ", self.entry)
        self.assertIn("plugin_version = ", self.entry)
    ...
    def test_inflight_magnet_ed2k_claims_block_other_sources_but_not_themselves(self):
        method = self.fence.split("    def _pending_reservations(", 1)[1].split(
    ...
    def test_poll_never_uses_cancelled_old_task_id(self):
        method = self.fence.split("    def _poll_offline_source(", 1)[1].split(
