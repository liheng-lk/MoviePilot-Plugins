...
def test_recall_guard_parses_and_is_above_existing_hardening():
    ...
    assert guard < hardening < runtime


def test_explicit_wrong_season_cannot_stop_keyword_fallback():
    matcher = _method("_provider_candidate_matches", "_candidate_episode_hint_v1125")
    assert "expected_season" in matcher
    assert "_candidate_seasons_v1125(row)" in matcher
    assert "expected_season not in seasons" in matcher
    assert "return False" in matcher
    season = _method("_candidate_seasons_v1125", "_provider_candidate_matches")
    assert "_SEASON_MARKER_RE_V1125" in season
