...
def test_quality_suffix_episode_examples():
    cases = {"01~4K.mp4": 1, "02~4K.mp4": 2, "08～2160p.mkv": 8, "22~[4K][HEVC.AAC].mkv": 22, "001 ~ UHD.zh.ass": 1}
...
def test_quality_suffix_episode_does_not_guess_numeric_ranges_or_codecs():
    for path in ("01~04.mp4", "2160p.mkv", "264~4K.mp4", "Show.S01E02.mkv", "movie.2026.4K.mkv"):
...
def test_duplicate_unparsed_failure_notice_is_collapsed():
    text = (
        ...
        "示例：01~4K.mp4、02~4K.mp4、03~4K.mp4；分享内有 13 个媒体/字幕文件无法解析集号，未标记为已处理；"
...
def test_runtime_entry_installs_compat_patch_and_new_build():
    ...
    assert 'build_id = "20260905-r59"' in text
