...
def test_runtime_fix_parses_and_is_wired_into_final_gying_chain():
    ...
    assert "from .runtime_fix_v1113 import GuangYaRuntimeFixV1113Mixin" in ui
...
def test_xunlei_invalid_captcha_chinese_error_forces_one_refresh_without_init_json_gate():
    ...
    detector = patch.split("    def _xunlei_captcha_invalid_v1113", 1)[1].split("    def _xunlei_get(", 1)[0]
    assert "验证码" in patch
    assert "验证码无效" in patch
    ...
    assert "已自动刷新运行时验证并重试当前分享" in method
