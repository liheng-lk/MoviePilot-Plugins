...
SAFETY = (ROOT / "plugins.v3" / "guangyatransferassistant" / "multisource_v180.py").read_text(encoding="utf-8")
...
def test_queued_is_not_resubmitted_after_restart():
    ...
    block = SAFETY.rsplit("    def _submit_offline_source(", 1)[1].split("    def _poll_offline_source(", 1)[0]
