...
def replace_once(path: Path, old: str, new: str) -> None:
    ...
    if new in text:
        return
    if old not in text:
        raise RuntimeError(f"contract marker missing: {path}")
    path.write_text(text.replace(old, new, 1), encoding="utf-8")


# v1.12.11 isolated class fixture: its direct base is now v1.12.12, while behavior remains the same.
...
# v1.12.10 historical contract still proves the physical season fence remains below all newer wrappers.
