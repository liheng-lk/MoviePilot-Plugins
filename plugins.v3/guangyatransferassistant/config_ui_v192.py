...
v1.9.3 最终配置页只让日常用户先看到“接管什么、资源从哪里来、怎么决策”。
节点发布页、PoW、代理、迅雷 device/captcha 等协议参数统一下沉到高级区域，避免把
运行时实现细节铺满主配置界面。
"""
...
class GuangYaConfigUiMixin:
    ...
    def get_form(self):
        ...
        save_path.update({
            "model": "save_path",
            "label": "光鸭目标文件夹",
            "hint": "可选择已有目录，也可直接输入完整路径",
            "persistent-hint": True,
        })

        basic = self._card(
            "基础",
            "先决定哪些 MoviePilot 订阅由光鸭接管，以及最终保存位置。",
            [
                ...
                {"component": "VRow", "content": [
                    ...
                    self._switch("create_media_folder", "媒体名文件夹", md=2),
        ...
        sources = self._card(
            "资源来源",
            "日常只需要配置来源和观影账号。观影迅雷秒传是最高优先级；失败后自动继续光鸭分享、Magnet、ED2K。",
            [
                {"component": "VRow", "content": [
                    self._textarea("channel_urls", "Telegram / 频道源", rows=3, md=6, hint="每行一个频道地址"),
                    self._textarea(
                        "magnet_api_sources",
                        "Magnet / ED2K 搜索接口",
                        rows=3,
                        md=6,
                        hint="可选。每行：名称|类型|地址|密钥；类型支持 tgsearch、limitless、json、torznab",
                    ),
                ]},
                {"component": "VRow", "content": [
                    self._switch("viewing_enabled", "启用观影", md=3),
                    self._switch("provider_auto_search", "缺资源自动搜索", md=3),
                    self._switch("viewing_auto_switch", "观影节点自动切换", md=3),
                ...
                {"component": "VRow", "content": [
                    self._field(
                        ...
                        hint="留空会从发布页/备用节点自动选择；填入后优先使用该节点。",
                        **{"persistent-hint": True},
                    ),
                ]},
                {"component": "VRow", "content": [
                ...
                {"component": "VRow", "content": [
                    self._textarea(
                        ...
                        "观影 Cookie（可选）",
                        rows=2,
                        hint="只有需要复用浏览器登录时才填写。Cookie 只绑定首选节点，不会在自动切换节点时跨域发送，也不会在状态/API 中回显。",
                    ),
                ]},
            ],
        )
        ...
        decision = self._card(
            "资源决策与云添加",
            "固定优先级：观影迅雷秒传 > 光鸭分享 > Magnet > ED2K。电视剧只处理真实缺集；低置信集号不会整包误存。",
            [
                {"component": "VRow", "content": [
                    self._switch("external_auto_dispatch", "新增来源自动处理", md=3),
                    self._switch("channel_external_auto_dispatch", "频道 Magnet/ED2K 候选", md=3),
                    self._field(
                        "source_priority",
                        "后续来源顺序",
                        cols=12,
                        md=6,
                        hint="迅雷秒传固定在最前；这里保持 guangya,magnet,ed2k 即可",
                        **{"persistent-hint": True},
                    ),
                ]},
                {"component": "VRow", "content": [
        ...
        advanced = self._card(
            "高级",
            "一般无需修改。这里只放节点发现、反爬验证、迅雷设备身份和抓取范围等协议参数。",
            [
                {"component": "VRow", "content": [
                    self._switch("viewing_auto_challenge", "自动完成观影计算验证", md=3),
                    self._switch("provider_proxy", "观影/外部搜索使用代理", md=3),
                    self._switch("proxy", "频道使用代理", md=3),
                    self._field("viewing_node_cache_minutes", "节点缓存(分钟)", cols=6, md=3, type="number", min="10", max="1440"),
                ]},
                {"component": "VRow", "content": [
                    self._textarea(
                        ...
                        rows=2,
                        md=6,
                        hint="默认 gying.page / gying.si；发布页不可用时会继续使用缓存与备用节点",
                    ),
                    self._textarea(
                        "viewing_node_urls",
                        "手动备用观影节点",
                        rows=2,
                        md=6,
                        hint="每行一个，可填写中文域名或 punycode；两者会自动归一为同一节点",
                    ),
                ]},
                {"component": "VRow", "content": [
                    self._field("provider_timeout", "外部请求超时(秒)", cols=6, md=3, type="number"),
                    self._field("provider_result_limit", "外部候选上限", cols=6, md=3, type="number"),
                    self._field("xunlei_flash_max_files", "迅雷单分享文件上限", cols=6, md=3, type="number", min="1", max="500"),
                ...
                {"component": "VRow", "content": [
                    self._field(
                        ...
                        cols=12,
                        md=6,
                        placeholder="只有手工 captcha_token 时才需要填写对应 device_id",
                    ),
                    self._field(
                        "xunlei_captcha_token",
                        "迅雷 captcha_token（通常自动获取）",
                        cols=12,
                        md=6,
                        type="password",
                        autocomplete="off",
                ...
                {"component": "VRow", "content": [
                    self._textarea(
                        ...
                        rows=2,
                        hint="自动初始化失败时，可从浏览器开发者工具复制 shield/captcha/init 的 JSON；不会在公开 API 回显。",
                    ),
                ]},
                {"component": "VRow", "content": [
                ...
                {"component": "VRow", "content": [
                    self._field("max_share_files", "单分享扫描上限", cols=6, md=3, type="number"),
                    self._field("ongoing_guard_days", "连载等待(天)", cols=6, md=3, type="number"),
                    self._field("summary_cron", "每日摘要 Cron", cols=12, md=3),
                    self._switch("clear_inventory", "保存时清空去重记录（一次性）", md=3),
                ]},
            ],
        )
