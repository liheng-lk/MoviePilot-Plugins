...
v1.12.5 收口观影迅雷召回与 Push/Pull 调度：目标卡排序后再展开 downurl，关键词按当前媒体与真实缺集逐级降级并复用短期 SearchBundle；5 分钟仅消费频道 Push，常规 GYING 由更新日历/冷却统一 Pull；频道已到达资源不受日期门禁；新订阅频道优先；04:10 仅对频道后剩余缺口强制补漏，并加强故障退避、验证码 circuit、线程局部上下文与异步 trigger 原子性。
