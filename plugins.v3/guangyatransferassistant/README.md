...
- **运行健康诊断**：检查观影/Provider 连通性、固定订阅是否具备搜索前置条件、迅雷秒传预检；不会为固定订阅重新发起具体资源搜索。
- **搜索缺失资源**：显式访问 GYING / 外部 Provider，为当前固定订阅查找迅雷、Magnet、ED2K 精确候选。
- **运行自检**：检查插件自身接管、搜索门禁、原生云添加等本地运行状态。

这样排障时不会因为点“诊断”就额外打一轮 GYING，也不会把“当前媒体没资源”误认为插件本身异常。
