## v2.1.5-r104 — GYING 会话收敛、续作身份桥接与拆包诊断（Controlled Real-World Beta）

本轮基于 2.1.5-r103 的真实 MoviePilot 日志继续热修，不改变固定来源优先级和最终写盘硬门禁。

### r104 实机热修

- GYING 不再让不同订阅关键词并发进入同一节点的 PoW/机器人挑战事务；搜索、挑战、重试按进程级事务锁收敛。
- PoW / `browser_verified` 等挑战态只复用当前进程内同一个活 `requests.Session`，不持久化、不跨新 Session 复活；连续挑战失败时主动废弃失效会话，下次请求干净重建。
- 对 MoviePilot 将编号续作作为独立作品 S01、发布方仍使用“基础标题 + 同号 S0N”的情况增加严格桥接，例如《我们的少年时代2》S01 ↔ 《我们的少年时代 S02》；普通作品 S01/S02 冲突仍硬拒绝。
- `alias_match` 日志只在真实身份匹配证据存在时为 true，不再因为“有别名列表”就误报。
- 迅雷缺集拆包继续保持 `AUTO_SELECT_CONFIDENCE = 0.90`，不按文件顺序猜集号；当 planner ambiguous 时输出有限的文件级 Episode Resolver 诊断，便于判断资源本身缺集还是命名漏解析。

### r103 基线：频道后观影补搜、汇总去重

本版修复实机中频道批次检查了大量订阅，却出现大量“本地暂无资源”、`外部搜索无结果=0`，观影/GYING 实际没有进入同轮后备链的问题。

### 调度

- 频道仍然先执行，频道 worker 本身不直接同步访问 GYING。
- 频道阶段结束后重新计算真实缺集、更新日历与外部检索冷却，只让仍应处理的订阅进入 `观影迅雷秒传 > 光鸭直接转存 > Magnet > ED2K` 完整来源链。
- 首页“手动刷新”使用相同的受控两阶段；不会把所有固定订阅无条件打到观影。
- `/gycheck` 仍保留人工强制完整检查语义。

### 汇总

- 同一订阅即使在频道阶段、观影阶段或多个候选产生多个 run，通知中只保留一个最终结果。
- `本轮检查` 改为实际订阅数口径，不再把同一 #58/#200 重复累计。
- 通知标题只显示一次，正文不再重复 `⚠️ 光鸭转存检查汇总`。
- `MEDIA_IDENTITY_UNCONFIRMED` 会附带有限的真实顶层标题/文件证据，便于继续定位。

### 季号兼容

- 对《我们的少年时代2》这类 MoviePilot 将续作作为新作品 S1、发布方仍写 S02 的情况增加窄兼容。
- 只有订阅标题明确以同一个 2~9 结尾、MoviePilot 期望 S1、资源显式季号与该数字一致、真实标题强匹配时才放行。
- 普通作品的 S01/S02 冲突、错误季资源、弱证据资源继续硬拒绝。
## v2.1.4-r102 — 频道标题归一化与调度可观测性（Controlled Real-World Beta）

本版针对实机频道日志中“资源已经发现，但标题被 Telegram 镜像 Forwarded 文案污染、且扫描后无法判断是否真正进入订阅调度”的问题做收口。

### 修复

- `光鸭云盘资源频道 Forwarded from 光鸭云盘资源频道 早春晴朗 NF` 统一归一化为 `早春晴朗`，Resource Inbox 与 legacy 频道入口使用同一干净标题。
- 只处理已知光鸭频道 Forwarded 模板，不做编辑距离、包含式或其它通用模糊匹配，最终媒体身份门禁不放宽。
- 补齐 `更新至N集`、`更新到N集`、`更至N集` 与 `SxxEyy-Ezz` 的季集元数据，避免热更模板只发现链接但缺少可用 Episode Hint。
- 原 `【媒体匹配】` 日志改名为 `【媒体解析】`：该阶段只是从频道消息抽取标题/季集，并未与 MoviePilot 订阅做最终匹配。
- 手动“刷新频道”、5 分钟 tick、固定订阅批量入队新增明确的 `调度入口/出口/订阅调度` 日志，可直接判断是否因为自动处理开关、固定订阅为空或异步队列未触发而停止。
- r101 目标感知检索冷却、r100 Emby 实际缺集恢复、最终物理文件门禁与来源优先级保持不变。
