...
class ShukGuangYaDisk(
    ...
    def _save_config(self, config_payload: dict) -> Dict[str, Any]:
        try:
            ...
            new_config = {
                ...
                "client_id": ((config_payload.get("client_id") or self._client_id or GuangYaClient.DEFAULT_CLIENT_ID).strip() or GuangYaClient.DEFAULT_CLIENT_ID),
    ...
    def get_api(self) -> List[Dict[str, Any]]:
        ...
        apis.extend([
            {"path": "/login/sms/send", "endpoint": self.send_sms_code, "auth": "bear", "methods": ["POST"], "summary": "发送光鸭云盘短信验证码", "response_model": GuangYaActionResponse},
            {"path": "/login/sms/verify", "endpoint": self.verify_sms_login, "auth": "bear", "methods": ["POST"], "summary": "校验短信验证码并完成登录", "response_model": GuangYaActionResponse},
        ])
    ...
    def send_sms_code(self, payload: dict) -> Dict[str, Any]:
        ...
        if not self._client:
            self._client = GuangYaClient(access_token=None, refresh_token=None, client_id=self._client_id, device_id=self._device_id)
            self._device_id = self._client.device_id
        result = self._client.request_sms_code(phone_number=phone, captcha_token=str(payload.get("captcha_token") or "").strip())
    ...
    def verify_sms_login(self, payload: dict) -> Dict[str, Any]:
        ...
        result = self._client.signin_by_sms(phone_number=phone, verification_id=verification_id, verification_code=code, captcha_token=captcha_token)
        ...
        return {"success": True, "message": "短信登录成功，光鸭云盘存储已启用", "device_id": self._device_id, "enabled": True}
    ...
    def poll_login(self) -> Dict[str, Any]:
        ...
        temp_client = GuangYaClient(access_token=None, refresh_token=None, client_id=self._client_id, device_id=self._device_id)
        result = temp_client.poll_device_code(self._device_code)
        if result and result.get("waiting"):
            return {"success": False, "message": result.get("message") or "等待扫码确认...", "waiting": True, "stage": "authorization_pending"}
        if not result or not result.get("access_token"):
            return {"success": False, "message": "已扫码，等待光鸭返回登录令牌...", "waiting": True, "stage": "token_pending"}
        ...
        return {"success": True, "message": "扫码登录成功，登录信息已保存", "device_id": self._device_id, "enabled": True, "has_access_token": bool(self._access_token), "has_refresh_token": bool(self._refresh_token)}
