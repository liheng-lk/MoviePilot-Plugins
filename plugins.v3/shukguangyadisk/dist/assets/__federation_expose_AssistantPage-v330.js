...
export default defineComponent({
  ...
  setup(props,{emit}) {
    ...
    function toggleGroup(group){const key=groupKey(group);const next=new Set(expandedGroups.value);next.has(key)?next.delete(key):next.add(key);expandedGroups.value=next;}
    ...
    async function loadStatus(silent=true){try{const r=await getApi(props,'/organize/monitor/status');if(!r?.success)throw new Error(r?.message||'读取失败');status.value=r?.data?.status||{};history.value=r?.data?.history||[];folderHistory.value=r?.data?.folder_history||[];mp.value=r?.data?.mp||mp.value;if(!silent&&r?.message)setMsg(r.message);}catch(e){if(!silent)setMsg(e?.message||'读取运行状态失败','error');}}
    ...
    const flatHistory=()=>h('div',{class:'gya-history'},history.value.length?history.value.map(row=>h('div',{class:'gya-history-row'},[
      h('div',[h('b',row.time||'-')]),
    ...
    const groupedHistory=()=>folderHistory.value.length?h('div',{class:'gya-groups'},folderHistory.value.map(group=>{
      const key=groupKey(group), expanded=expandedGroups.value.has(key), metrics=groupMetrics(group);
      return h('div',{class:'gya-group',key},[
        h('button',{class:'gya-group-head',onClick:()=>toggleGroup(group)},[
          h('span',{class:'gya-group-arrow'},expanded?'▼':'▶'),
          h('div',{class:'gya-group-main'},[
            h('b',group.group_name||'-'),
            h('small',group.group_path||''),
            group.latest_time?h('div',{class:'gya-group-time'},`最近：${group.latest_time}`):null
          ]),
          h('div',{class:'gya-group-metrics'},metrics.map(item=>h('span',{class:`gya-chip ${item.cls}`},item.text)))
        ]),
        expanded?h('div',{class:'gya-group-body'},[
          ...
          !(group.rows||[]).length?h('div',{class:'gya-card-sub'},'该目录当前只有状态记录，暂无最近文件流水'):null
        ]):null
      ]);
    })):flatHistory();
    ...
    const organizer=()=>h('div',{class:'gya-shell'},[
      h('div',{class:'gya-head'},[
        h('div',[h('div',{class:'gya-title'},'自动整理监控'),h('div',{class:'gya-sub'},'按子目录完整扫描并立即加入 MoviePilot 队列；扫描后续目录时，前一目录可同时整理。分类、命名和目标目录仍全部由 MoviePilot 决定。')]),
        h('span',{class:'gya-badge'},'v3.4.0 preview')
      ]),
      h('div',{class:'gya-body'},[
        h('div',{class:'gya-card'},[
          h('div',{class:'gya-card-title'},'1. 监控目录'),
          h('div',{class:'gya-card-sub'},'监控根的一个直接子文件夹作为一个完整整理批次；目录内文件连续提交，便于整季/短剧统一排队和查看历史。'),
        ...
        h('div',{class:'gya-card'},[
          h('div',{class:'gya-card-title'},'2. 队列与扫描参数'),
          h('div',{class:'gya-card-sub'},'单次提交上限同时作为 MoviePilot 待整理背压上限；当前目录优先续完，后续目录不会抢占剩余队列容量。'),
          h('div',{class:'gya-grid'},[
            ...
            h('div',{class:'gya-field'},[h('label','MP 队列/单轮上限'),h('input',{class:'gya-input',type:'number',min:'1',max:'500',value:batchSize.value,onInput:e=>batchSize.value=e.target.value})])
          ])
        ]),
        h('div',{class:'gya-note'},[
          ...
          h('br'),
          h('span','插件只负责远程发现、稳定性、历史门控、识别上下文和队列调度；目标目录、重命名、覆盖、刮削仍由 MP 决定。')
        ]),
        ...
        h('div',{class:'gya-card'},[
          ...
          h('div',{class:'gya-stats'},[
            ['扫描文件',status.value.inventory],['子目录',`${status.value.groups_scanned||0}/${status.value.groups_discovered||0}`],['本轮提交',status.value.submitted],['整理中',status.value.state_inflight??status.value.inflight],['队列槽位',status.value.queue_slots],
            ['已完成',status.value.state_completed??status.value.completed],['重试等待',status.value.state_retry_wait??status.value.retry_wait],['MP 门控',status.value.state_blocked??status.value.blocked],['等待容量',status.value.capacity_wait],['提交异常',status.value.failed]
          ...
          h('div',{class:'gya-statusline'},[
            ...
            status.value.scan_in_progress?h('span',`正在扫描：${status.value.current_group_name||status.value.current_group||'-'}`):null,
            h('span',`上次扫描：${status.value.last_scan||'尚未扫描'}`),
            status.value.duration_ms!=null?h('span',`耗时：${status.value.duration_ms} ms`):null,
            status.value.partial?h('span','上次为部分扫描，未执行破坏性 inventory 清理'):null,
            status.value.truncated?h('span','扫描达到安全上限，当前未完整目录不会提交'):null
          ])
        ]),
        h('div',{class:'gya-card'},[
          h('div',{class:'gya-card-title'},'按子目录整理历史'),
          h('div',{class:'gya-card-sub'},'一个监控根直接子目录对应一个批次视图。折叠时看整体结果，展开后查看该目录的文件级 queued / completed / failed 流水。'),
          groupedHistory()
        ])
      ])
    ]);
