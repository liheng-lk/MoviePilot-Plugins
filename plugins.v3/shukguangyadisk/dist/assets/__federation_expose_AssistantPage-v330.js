...
export default defineComponent({
  ...
  setup(props,{emit}) {
    ...
    const statusDot=computed(()=>{
      ...
      if(runtimePhase.value==='handoff'||blocked.value>0)return 'warn';
    ...
    const organizer=()=>h('div',{class:'gya-shell'},[
      h('div',{class:'gya-head'},[
        h('div',[h('div',{class:'gya-title'},'自动整理监控'),h('div',{class:'gya-sub'},'资源发现 → MoviePilot 识别/预览 → 安全校验 → 真实整理 → MP历史落库。电影容器严格单任务，电视剧保留整季事务。')])
      ]),
      h('div',{class:'gya-body'},[
        ...
        h('div',{class:'gya-card'},[
          ...
          h('div',{class:'gya-card-sub'},'电影容器一次只处理一个资源；电视剧/Season 作为一个文件夹事务整体预检并连续整理。'),
        ...
        h('div',{class:'gya-card'},[
          ...
          status.value.current_task_path?h('div',{class:'gya-statuspath'},[
            h('b',runtimePhase.value==='handoff'?'旧版本遗留任务正在安全收尾：':'当前资源：'),
          ...
          !status.value.last_transfer_history_id&&['scanning','queued','transferring','handoff'].includes(runtimePhase.value)?h('div',{class:'gya-card-sub',style:{marginTop:'6px',marginBottom:'0'}},'当前尚未出现 MoviePilot 整理历史编号，说明该任务还没有完成真实整理结算；仅看到识别/目标路径日志不等于已经整理完成。'):null
        ]),
        h('div',{class:'gya-card'},[
