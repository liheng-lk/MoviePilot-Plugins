...
.gya-alerts{display:grid;gap:6px;margin-top:8px}.gya-alert{padding:8px 9px;border-radius:8px;border:1px solid rgba(var(--v-theme-on-surface),.09);font-size:10px}.gya-alert b{margin-right:6px}.gya-alert.info{background:rgba(59,130,246,.08);border-color:rgba(59,130,246,.2)}.gya-alert.warn{background:rgba(245,158,11,.08);border-color:rgba(245,158,11,.25)}.gya-alert.error{background:rgba(239,68,68,.08);border-color:rgba(239,68,68,.25)}.gya-tips{margin:8px 0 0;padding-left:16px;font-size:10px;line-height:1.65;opacity:.82}
...
export default defineComponent({
  ...
  setup(props,{emit}) {
    ...
    const diagnostics=ref({}),alerts=ref([]),tips=ref([]),scanSummary=ref({}),uploadSummary=ref({});
    ...
    function applyDiagnostics(d={}){diagnostics.value=d||{};alerts.value=d?.alerts||[];tips.value=d?.tips||[];scanSummary.value=d?.last_scan_summary||{};uploadSummary.value=d?.last_upload_summary||{};}
    ...
    async function loadDiagnostics(silent=true){try{const r=await getApi(props,'/organize/monitor/diagnostics');if(!r?.success)throw new Error(r?.message||'读取失败');applyDiagnostics(r?.data||{});}catch(e){if(!silent)setMsg(e?.message||'读取诊断失败','error');}}
    async function loadStatus(silent=true){try{const r=await getApi(props,'/organize/monitor/status');if(!r?.success)throw new Error(r?.message||'读取失败');status.value=r?.data?.status||{};history.value=r?.data?.history||[];mp.value=r?.data?.mp||mp.value;await loadDiagnostics(true);if(!status.value?.queue_guard_active&&messageKind.value==='error'&&isLegacyQueueError(message.value))setMsg('');if(!silent)setMsg(status.value?.graceful_stop_message||status.value?.queue_guard_message||r?.message||'状态已刷新',runtimePhase.value==='handoff'||runtimePhase.value==='draining'||status.value?.queue_guard_active?'warn':'ok');}catch(e){if(!silent)setMsg(e?.message||'读取运行状态失败','error');}}
    ...
    const organizer=()=>h('div',{class:'gya-shell'},[
      ...
      h('div',{class:'gya-body'},[
        ...
        h('div',{class:'gya-card'},[
          h('div',{class:'gya-card-title'},'工程诊断'),
          h('div',{class:'gya-card-sub'},'按故障等级展示当前风险，并给出扫描/上传摘要，便于快速定位。'),
          h('div',{class:'gya-stats'},[
            ['扫描阶段',scanSummary.value.runtime_phase||'-'],['扫描目录',scanSummary.value.scan_dirs_scanned||0],['资源目录',scanSummary.value.scan_resource_dirs||0],['剩余目录',scanSummary.value.scan_cursor_remaining_dirs||0],
            ['上传阶段',uploadSummary.value.last_stage||'-'],['上传状态',uploadSummary.value.last_status||'-'],['上传错误',uploadSummary.value.recent_error_count||0],['诊断版本',diagnostics.value.diagnostic_version||'-']
          ].map(([k,v])=>h('div',{class:'gya-stat'},[h('span',k),h('b',String(v||0))]))),
          h('div',{class:'gya-alerts'},(alerts.value||[]).length?(alerts.value||[]).map(row=>h('div',{class:`gya-alert ${row.severity||'info'}`},[
            h('b',`[${String(row.severity||'info').toUpperCase()}]`),
            h('span',row.message||'-'),
            row.code?h('small',{style:{display:'block',opacity:.72,marginTop:'4px'}},`#${row.code} · ${row.action||''}`):null
          ])):h('div',{class:'gya-card-sub'},'当前无额外告警')),
          (tips.value||[]).length?h('ul',{class:'gya-tips'},(tips.value||[]).map(t=>h('li',t))):null
        ]),
        h('div',{class:'gya-card'},[
