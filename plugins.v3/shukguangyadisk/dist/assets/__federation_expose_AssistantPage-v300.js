...
export default defineComponent({
  name: 'GuangyaCloudAssistantV300',
  props: {
  ...
  setup(props, { emit }) {
    ...
    const load = async () => {
      ...
      try {
        ...
        const mod = await import('./__federation_expose_AssistantPage-dev.js?v=3.0.0');
        InnerPage.value = mod.default;
      } catch (err) {
        console.error('[光鸭云盘助手 V3] Page chunk load failed', err);
    ...
    onMounted(load);
    onErrorCaptured((err) => {
      console.error('[光鸭云盘助手 V3] Page runtime error', err);
    ...
    return () => {
      ...
      if (loading.value) {
        return h('div', { style: boxStyle }, [h('strong', '光鸭云盘助手'), h('span', '正在加载 MoviePilot V3 插件界面…')]);
      }
      return h('div', { style: boxStyle }, [
        ...
        h('div', { style: { fontSize: '10px', opacity: '.45' } }, '光鸭云盘助手 v3.0.0 · MoviePilot V3 前端故障隔离模式'),
      ]);
    };
  },
});
