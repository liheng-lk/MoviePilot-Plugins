import BasePage from './__federation_expose_AssistantPage-v352.js?v=3.10.0';
    ...
    const control=()=>h('div',{class:'gya390'},[
      ...
      h('div',{class:'gya390-card'},[
        h('div',{class:'gya390-head'},[
          h('div',[
            h('div',{class:'gya390-title'},'整理监控控制 · v3.10.0'),
        ...
        h('div',{class:'gya390-progress'},[
          ...
          h('div',`调度探测：${status.value?.resource_dispatch_probe_count||0} ｜ 跳过等待：${status.value?.resource_dispatch_skipped_waiting||0} ｜ 连续消费：${status.value?.worker_continuation_scheduled?'已续跑':(status.value?.worker_continuation_reason||'-')}`),
