...
export default defineComponent({
  ...
  setup(props,{emit}){
    ...
    const stageText=computed(()=>{const no=Number(status.value?.scan_stage_no||0),total=Number(status.value?.scan_stage_total||6);return no?`${no}/${total} ${status.value?.scan_stage||''}`:(status.value?.scan_stage||'-');});
    function setMsg(text,kind='ok'){message.value=text||'';messageKind.value=kind;}
    async function loadStatus(silent=true){try{const r=await getApi(props,'/organize/monitor/status');if(!r?.success)throw new Error(r?.message||'读取扫描状态失败');status.value=r?.data?.status||{};}catch(e){if(!silent)setMsg(e?.message||'读取扫描状态失败','error');}}
    async function action(kind,path){busy.value=kind;setMsg('');try{const r=await postApi(props,path,{});if(!r?.success)throw new Error(r?.message||'操作失败');setMsg(r?.message||'操作已提交',r?.data?.full_scan_paused?'warn':'ok');await loadStatus(true);}catch(e){setMsg(e?.message||'操作失败','error');}finally{busy.value='';}}
    ...
    const control=()=>h('div',{class:'gya376'},[
      h('style',css),h('div',{class:'gya376-card'},[
        h('div',{class:'gya376-head'},[
          h('div',[h('div',{class:'gya376-title'},'整理扫描控制 · 双通道'),h('div',{class:'gya376-sub'},'增量：pending → 已知资源变化 → discovery 推进一页；全量：从监控根目录完整遍历，整理期间暂停并自动续页。')]),
        ...
        h('div',{class:'gya376-state'},[
          ['当前模式',scanMode.value],['扫描阶段',stageText.value],['全量页数',status.value?.full_scan_pages||0],['累计目录',status.value?.full_scan_dirs||0],['剩余游标',status.value?.full_scan_remaining_dirs||0]
