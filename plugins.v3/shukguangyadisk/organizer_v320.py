...
class GuangYaOrganizerMixin(_BaseOrganizerMixin):
    ...
    def api_organize_execute(self, payload: dict) -> Dict[str, Any]:
        """执行计划；只要本批实际包含移动任务，就在执行后安全清理旧的空子目录。"""
        payload = payload or {}
        plan_id = str(payload.get("plan_id") or "")
        plan = dict((getattr(self, "_organize_plans", {}) or {}).get(plan_id) or {})
        move_planned = any(
            row.get("status") == "ready" and str(row.get("operation") or "move") == "move"
            for row in (plan.get("items") or [])
        )
        source_root = str(plan.get("source_path") or "")
        result = super().api_organize_execute(payload)
        if move_planned and source_root:
            try:
                removed = self._cleanup_empty_source_dirs(source_root)
                data = result.setdefault("data", {}) if isinstance(result, dict) else {}
                data["cleaned_empty_dirs"] = removed
                if removed:
                    logger.info(
                        "【光鸭云盘助手】【完整整理】移动完成后已清理 %s 个空旧目录: %s",
                        removed, source_root,
                    )
            except Exception as err:
                logger.warning("【光鸭云盘助手】【完整整理】空旧目录清理失败，不影响已完成文件: %s", err)
        return result
