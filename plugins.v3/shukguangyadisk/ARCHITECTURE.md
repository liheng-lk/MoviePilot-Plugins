...
`FinalMonitor 发现/持久队列 → QueueRecovery 私有 Worker → TransferChain(background=False) → MoviePilot 识别/命名/移动 → 光鸭远端终态确认 → 状态机终态`

历史 `v341x/v35x/v36x` 模块可以继续提供已经验证的识别上下文、命名保护、远端确认等兼容能力，但必须遵守：

- 不得覆盖 FinalMonitor 的资源队列调度权；
- 不得删除、停止或阻塞 MoviePilot 宿主全局队列任务；
- 不得把 waiting 资源当成全局停止条件；
- 不得因为一个 waiting 资源阻塞其它 ready 资源；
- 不得依赖下一次 heartbeat 才继续消费已经存在的持久队列；
- 所有“迁移”默认 fail-safe：无法证明任务所有权时只能观察，不能修改。

调度器每轮最多提交一个真实 Worker 任务，但允许在同一轮有界探测多个候选，跳过仍在等待的资源，直到找到一个 ready 资源或确认当前没有 due 资源。这样保持单 Worker 的同时避免队首饥饿。
