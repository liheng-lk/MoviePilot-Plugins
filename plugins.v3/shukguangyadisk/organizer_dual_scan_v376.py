...
from .organizer_engine_v360 import GuangYaOrganizerEngineV360Mixin as _EngineMixin
...
def install_dual_scan_v376() -> None:
        ...
        try:
            ...
            if data.get("cycle_complete"):
                ...
                self.save_data(
                    _FULL_LAST_KEY,
                    {
                        "monitor_path": _norm(self, getattr(self, "_organize_monitor_path", "")),
    ...
    def start_full(self, trigger: str = "manual") -> Dict[str, Any]:
        ...
        try:
            ...
            _trace(self, 2, "准备", f"目录={root}；已重置 discovery 游标，直到 cycle_complete 才结束")
        finally:
            _restore_context(self, old)
    ...
    def stop_full(self, trigger: str = "manual") -> Dict[str, Any]:
        ...
        return {
            ...
            "data": {"scan_id": session.get("scan_id"), "full_scan_active": False},
        }
    ...
    def tick_wrapped(self) -> None:
        ...
        if _load_full(self).get("active"):
            last_step = float(getattr(self, "_v376_full_last_step_mono", 0) or 0)
            if last_step and now - last_step < _FULL_STEP_MIN_GAP:
                return
        ...
        last_tick = float(getattr(self, "_v360_last_tick", 0) or 0)
